...
import { stageChangeHtml, runUpdateHtml } from "./emailTemplates";

const callableOpts = { memory: "256MB" as const, timeoutSeconds: 30 };

/**
 * Callable (admin only): send test emails for stage change + run update to the given address.
 */
export const sendTestEmails = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    const stageHtml = stageChangeHtml(
      opts,
      "Hype GTR – Interstellar",
      "Perth Run #7 – March 2026",
      "Neck Carve & Routing",
      "Finishing"
    );
    ...
    await sendEmail(to, stageSubject, stageHtml, stageText);

    // 2. Run update test
    const runSubject = "Perth Run #7 – March 2026: Week 3 progress";
    const runHtml = runUpdateHtml(
      opts,
      "Perth Run #7 – March 2026",
      "Week 3 progress",
      "The Factory Team",
    ...
    await sendEmail(to, runSubject, runHtml, runText);
