...
import { customShopThankYouHtml, customShopStaffNotifyHtml } from "./emailTemplates";
 ...
 */
export const onCustomShopRequestCreated = functions
  .runWith(opts)
  ...
  .onCreate(async (snap, context) => {
    ...
    const submitterEmail = (data.submitterEmail as string) || "";
    const submitterName = (data.submitterName as string) || null;
    const guitarDescription = (data.guitarDescription as string) || "";
    ...
    const thankYouHtml = customShopThankYouHtml(templateOpts, requestNumber, viewRequestUrl);
    ...
    await sendEmail(submitterEmail, thankYouSubject, thankYouHtml, thankYouText, { noCc: true });
    ...
    const staffHtml = customShopStaffNotifyHtml(
      templateOpts,
      submitterName,
      submitterEmail,
      requestNumber,
    ...
    await sendEmail(STAFF_EMAIL, staffSubject, staffHtml, staffText);
