 ...
 */
exports.setClientRole = functions.runWith(callableOpts).https.onCall(async (data, context) => {
    ...
    if (callerUid !== uid && callerRole !== "staff" && callerRole !== "admin") {
        throw new functions.https.HttpsError("permission-denied", "Only staff and admins can set roles for other users");
    }
    ...
    if (callerUid === uid) {
        try {
            ...
            throw new functions.https.HttpsError("internal", `Failed to set client role: ${error.message}`);
        }
    }
    else {
...
//# sourceMappingURL=setClientRole.js.map